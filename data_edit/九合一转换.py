...
# ===================== 配置参数（九合一+正方形优先） =====================
# 源图片目录（大火花）
src_img_dir = r"C:\Users\dxzw-xm16\Desktop\增强火花\原来部分\images"
# 合并后图片保存目录
dst_img_dir = r"C:\Users\dxzw-xm16\Desktop\增强火花\原来部分\images_1"
# 标注文件目录
label_dir = r"C:\Users\dxzw-xm16\Desktop\增强火花\原来部分\labels"
# 图片格式（支持多格式）
img_exts = [".jpg", ".png"]
# 空白区域填充颜色（白色）
fill_color = (255, 255, 255)
# 九合一默认布局（3×3，天然最接近正方形）
...
def get_group_cell_size(img_pairs):
    """
    计算当前组9张图片的单元格尺寸（最大宽、最大高）
    :param img_pairs: 9个图片-标注字典的列表
...
def convert_label_coords(label_lines, img_size, cell_pos, cell_size, grid_rows, grid_cols):
    """
    转换标注坐标到合并后的大图（3×3布局，无缩放，仅偏移）
    ...
    :param grid_rows: 网格行数（3）
    :param grid_cols: 网格列数（3）
    :return: 转换后的标注行列表
    """
    converted_lines = []
    ...
    for line in label_lines:
        ...
        w_big_rel = w_abs / big_w
        h_big_rel = h_abs / big_h
...
def merge_nine_imgs_square(img_pairs, dst_img_path, dst_label_path):
    """
    合并9张图片（3×3正方形布局+保持原尺寸+居中）+ 转换标注
    :param img_pairs: 9个图片-标注字典的列表
    ...
    print(f"  布局：{GRID_ROWS}行{GRID_COLS}列 | 大图尺寸：{big_w}×{big_h}（宽高比：{big_w/big_h:.2f}）")
    ...
    for idx, pair in enumerate(img_pairs):
        ...
        # 转换标注坐标
        converted_labels = convert_label_coords(label_lines, img_size, cell_pos, (cell_w, cell_h), GRID_ROWS, GRID_COLS)
...
if __name__ == "__main__":
    ...
    dst_label_dir = os.path.join(os.path.dirname(dst_img_dir), "大火花-九个合一个-合并完-labels")
    create_dir_if_not_exist(dst_label_dir)

    # 获取图片-标注对应关系
    img_label_pairs = get_img_label_pairs(src_img_dir, label_dir, img_exts)
    total_groups = len(img_label_pairs) // 9  # 9张一组
    remaining_imgs = len(img_label_pairs) % 9

    print(f"开始处理九合一（3×3正方形布局），共{len(img_label_pairs)}张图片，分为{total_groups}组")
    # 按每9张分组处理
    for group_idx in range(total_groups):
        start_idx = group_idx * 9
        end_idx = start_idx + 9
        ...
        # 合并图片+转换标注（3×3正方形布局）
        print(f"\n处理第{group_idx + 1}组：")
        merge_nine_imgs_square(current_group, dst_img_path, dst_label_path)
        print(f"  保存图片：{dst_img_path}")
        print(f"  保存标注：{dst_label_path}")
    
    # 提示剩余图片
    if remaining_imgs > 0:
        remaining_names = [p["img_name"] for p in img_label_pairs[-remaining_imgs:]]
        print(f"\n注意：剩余{remaining_imgs}张图片未处理（不足9张）：{remaining_names}")
